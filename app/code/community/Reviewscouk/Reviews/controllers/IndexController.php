...
class Reviewscouk_Reviews_IndexController extends Mage_Core_Controller_Front_Action
{
	...
	public function feedAction()
	{
        ...
        $useGrouped = $this->helper->config('advanced/reviews_use_group_product_sku', Mage::app()->getStore());
		...
		if ($productFeedEnabled)
		{
			...
			if(!$saveCached)
			{
				...
				foreach ($products as $prod)
				{
                    ...
                    // If using grouped, don't show invisible products
                    if(!$useGrouped || $product->getVisibility() > 1){
                        $productFeed .= "<item>
				...
				$cache->save($productFeed, "feed", array("reviews_cache"), 86400);
			}
			else
			{
